...
class ExploreCategoryScreen extends StatelessWidget {
  ...
  String get _categoryKey {
    switch (category) {
      case ExploreCategoryType.hospitals: return 'hospitals';
      case ExploreCategoryType.university: return 'university';
      case ExploreCategoryType.shopping: return 'shopping';
      case ExploreCategoryType.cafesAndFarms: return 'cafesAndFarms';
    }
  }
  ...
  String get _heroImage {
    switch (category) {
      case ExploreCategoryType.hospitals: return 'assets/images/explore_hospitals1.png';
      case ExploreCategoryType.university: return 'assets/images/explore_university1.png';
      case ExploreCategoryType.shopping: return 'assets/images/explore_shopping1.png';
      case ExploreCategoryType.cafesAndFarms: return 'assets/images/explore_cafes_farms1.png';
    }
  }

  IconData get _categoryIcon {
    switch (category) {
      case ExploreCategoryType.hospitals: return Icons.local_hospital_outlined;
      case ExploreCategoryType.university: return Icons.school_outlined;
      case ExploreCategoryType.shopping: return Icons.shopping_bag_outlined;
      case ExploreCategoryType.cafesAndFarms: return Icons.local_cafe_outlined;
    }
  }
  ...
  List<Place> get _popularPlaces => _categoryPlaces.take(3).toList();
                        ...
                        SizedBox(
                          ...
                          child: ListView.separated(
                            ...
                            itemBuilder: (context, index) {
                              ...
                              return _PopularCard(place: place, onTap: () => _openPlace(context, place));
                            },
                          ),
                        ),
        ...
        children: [
          Positioned(
            ...
            child: IconButton(
              ...
              icon: Icon(AppData.isArabic ? Icons.arrow_back_ios_new : Icons.arrow_back_ios_new, color: Colors.white, size: 20),
            ),
          ),
          Center(
            child: Padding(
              padding: const EdgeInsets.only(top: 18),
              child: Text(title, style: const TextStyle(color: Colors.white, fontSize: 22, fontWeight: FontWeight.w500)),
            ),
          ),
        ],
      ),
...
class _SectionTitle extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    return Text(text, style: const TextStyle(color: Color(0xFF677191), fontSize: 16, fontWeight: FontWeight.w500, height: 1.7));
  }
}
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      ...
      child: Container(
        ...
        child: Stack(
          children: [
            Positioned.fill(child: Image.asset(place.imagePath, fit: BoxFit.cover)),
            Positioned(
              left: 0,
              right: 0,
              bottom: 0,
              child: Container(
                ...
                alignment: AppData.isArabic ? Alignment.centerRight : Alignment.centerLeft,
                child: Text(place.name, maxLines: 2, overflow: TextOverflow.ellipsis, style: const TextStyle(color: Colors.white, fontSize: 14, fontWeight: FontWeight.w600, height: 1.3)),
              ),
            ),
          ],
        ),
...
class _NearbyPlaceCard extends StatelessWidget {
  ...
  const _NearbyPlaceCard({required this.place, required this.icon, required this.iconBackgroundColor, required this.onTap});

  @override
  Widget build(BuildContext context) {
    String formattedDistance;
    if (place.distanceKm < 1) {
      formattedDistance = '${(place.distanceKm * 1000).toInt()} ${AppData.translate('m', 'م')}';
    } else {
          ...
          children: [
            ...
            Expanded(
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text(place.name, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(color: Color(0xFF1A485F), fontSize: 16, fontWeight: FontWeight.w500, height: 1.5)),
                  const SizedBox(height: 4),
                  Text(
                    '${place.availableSlots} ${AppData.translate('slots available', 'موقف متاح')}',
                    style: const TextStyle(color: Color(0xFF677191), fontSize: 14, fontWeight: FontWeight.w500, height: 1.4),
                  ),
                ],
              ),
            ),
            const SizedBox(width: 8),
            Text(formattedDistance, style: const TextStyle(color: Color(0xFF237D8C), fontSize: 16, fontWeight: FontWeight.w700, height: 1.7)),
          ],
        ),
      ),
    );
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Text(
        ...
        style: const TextStyle(color: Color(0xFF677191), fontSize: 14, fontWeight: FontWeight.w500),
      ),
    );
  }
}